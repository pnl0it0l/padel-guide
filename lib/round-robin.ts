 ...
 * 6. Limit total games to maxGames if specified
 *
 * @param teamIds - Array of team IDs to schedule
 * @param courtsAvailable - Number of courts available for games
 * @param maxGames - Optional maximum number of games to generate
...
export function generateRoundRobinSchedule(
  ...
  for (let round = 0; round < numRounds; round++) {
    ...
    for (let i = 0; i < halfSize; i++) {
      // Stop if we've reached the max games limit
    ...
    // Only add round if it has pairings
    ...
    // Stop creating rounds if we've reached max games
    if (maxGames && totalGames >= maxGames) {
      break;
    }
  }
...
export function calculateStandings(
  ...
  completedGames.forEach((game) => {
    ...
    team1Standing.pointsDiff =
      team1Standing.pointsFor - team1Standing.pointsAgainst;
    team2Standing.pointsDiff =
      team2Standing.pointsFor - team2Standing.pointsAgainst;
  });
